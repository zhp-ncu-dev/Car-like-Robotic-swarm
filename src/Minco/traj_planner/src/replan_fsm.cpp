...
void ReplanFSM::init(ros::NodeHandle &nh) {
  ...
  parking_sub_ = nh_.subscribe("/move_base_simple/goal", 1,
                               &ReplanFSM::ParkingCallback, this);
  swarm_traj_sub_ = nh_.subscribe("/broadcast_traj_to_planner", 100,
                                  &ReplanFSM::SwarmTrajCallback, this);

  exec_timer_ =
      nh_.createTimer(ros::Duration(0.02), &ReplanFSM::execFSMCallback, this);
  safety_timer_ = nh_.createTimer(ros::Duration(0.01),
                                  &ReplanFSM::checkCollisionCallback, this);
}

void ReplanFSM::OdomCallback(const nav_msgs::Odometry &msg) {
  Eigen::Vector3d center_pos(msg.pose.pose.position.x, msg.pose.pose.position.y,
  ...
  br.sendTransform(tf::StampedTransform(transform, msg.header.stamp, "map",
                                        "car_" + to_string(car_id_) + "_pos"));
}
...
void ReplanFSM::ParkingCallback(const geometry_msgs::PoseStamped &msg) {
  ...
  end_pt_ << target_x_, target_y_, target_yaw_, 1.0e-2;
...
void ReplanFSM::execFSMCallback(const ros::TimerEvent &e) {
  ...
  switch (exec_state_) {
    ...
    case EXEC_TRAJ: {
      ros::Time t_now = ros::Time::now();
      if (((cur_pos_ - init_state_.head(2)).norm() > 10.0 ||
           (t_now.toSec() - start_world_time_) > 2.5) &&
          (cur_pos_ - end_pt_.head(2)).norm() >
              5.0 /*&& !collision_with_othercars_*/) {
        changeFSMExecState(REPLAN_TRAJ, "FSM");
      }

      if ((collision_with_obs_ || collision_with_othercars_) &&
          t_now.toSec() - start_world_time_ >
              TIME_BUDGET /*+ 0.3*/)  // make sure the new trajectory have been
                                      // executed and then replan
      {
        changeFSMExecState(REPLAN_TRAJ, "FSM");
        collision_with_obs_ = false;
        collision_with_othercars_ = false;
      ...
      // reach end
      if ((cur_pos_ - end_pt_.head(2)).norm() < 0.5 &&
          abs(cur_yaw_ - end_pt_(2) < 0.15 && abs(cur_vel_) < 0.2)) {
        changeFSMExecState(WAIT_TARGET, "FSM");
        have_target_ = false;
        goto force_return;
      }
...
void ReplanFSM::checkCollisionCallback(const ros::TimerEvent &e) {
  ...
  if (exec_state_ == EXEC_TRAJ)
    collision_with_othercars_ =
        planner_ptr_->checkCollisionWithOtherCars(time_now);
}