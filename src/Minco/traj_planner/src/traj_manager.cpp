...
void TrajPlanner::init(const ros::NodeHandle& nh) {
  ...
  KinopathPub_ = nh_.advertise<visualization_msgs::Marker>(
      "car_" + to_string(car_id_) + "_kino_trajs", 10);
  Rectangle_poly_pub_ = nh_.advertise<decomp_ros_msgs::PolyhedronArray>(
      "car_" + to_string(car_id_) + "_polyhedrons", 1);
  minco_traj_pub_ = nh_.advertise<nav_msgs::Path>(
      "vis/car_" + to_string(car_id_) + "_minco_traj", 2);  // for visualizaion
  // MincoPathPub_ =
  // nh_.advertise<mpc::Trajectory>("/carla/ego_vehicle/trajectory", 1);
  MincoPathPub_ = nh_.advertise<kinematics_simulator::Trajectory>(
      "/car_" + to_string(car_id_) + "/trajectory", 1);  // for control
  TrajPathPub_ = nh_.advertise<swarm_bridge::Trajectory>(
      "/broadcast_traj_from_planner", 1);  // for swarm communication
  wholebody_traj_pub_ = nh_.advertise<visualization_msgs::Marker>(
      "vis/car_" + to_string(car_id_) + "wholebodyTraj", 2);
}
...
bool TrajPlanner::getKinoPath(Eigen::Vector4d& end_state, bool first_search) {
  ...
  std::cout << "start state: " << start_state.transpose()
            << " end_state: " << end_state.transpose() << std::endl;
  std::cout << "init ctrl: " << init_ctrl.transpose() << std::endl;
  int status =
      kino_path_finder_->search(start_state, init_ctrl, end_state, true);
  ...
  // std::cout<<"search time: "<<(searcht2-searcht1).toSec() *
  // 1000.0<<std::endl;
  ...
  if (status == path_searching::KinoAstar::NO_PATH) {
    ...
    kino_path_finder_->reset();
    status =
        kino_path_finder_->search(start_state, init_ctrl, end_state, false);
    if (status == path_searching::KinoAstar::NO_PATH) {
  ...
  bool middle_node_success =
      kino_path_finder_->searchTime(kino_trajs_, start_time_);
  auto time_searcht2 = high_resolution_clock::now();
  std::chrono::duration<double, std::milli> time_searchms =
      time_searcht2 - time_searcht1;
  cout << "time serach time: " << time_searchms.count() << " ms" << endl;
  // kino_path_finder_->getKinoNode(kino_trajs_);
  if (!middle_node_success) return false;

  return true;
}

void TrajPlanner::setInitStateAndInput(const Eigen::Vector4d& state,
                                       const double& start_time) {
  has_init_state_ = true;
  start_state_ = state;
  if (start_state_(3) < 0.05) start_state_(3) = 0.05;
...
void TrajPlanner::setInitStateAndInput(const double& t,
  ...
  Eigen::Vector2d start_pos =
      traj_container_.singul_traj[id].traj.getPos(t_bar);
  Eigen::Vector2d start_vel =
      traj_container_.singul_traj[id].traj.getdSigma(t_bar);
  Eigen::Vector2d start_acc =
      traj_container_.singul_traj[id].traj.getddSigma(t_bar);
  ...
  B << 0, -1, 1, 0;
  double init_steer =
      atan(singul * (start_acc.transpose() * B * start_vel)(0, 0) *
           car_wheelbase_ / pow(start_vel.norm(), 3));
  double init_acc =
      singul * (start_vel.transpose() * start_acc)(0, 0) / start_vel.norm();
...
void TrajPlanner::displayKinoPath(
    std::shared_ptr<plan_utils::KinoTrajData> kino_trajs) {
  ...
  sphere.header.stamp = line_strip.header.stamp = carMarkers.header.stamp =
      ros::Time::now();
  ...
  sphere.action = line_strip.action = carMarkers.action =
...
void TrajPlanner::displayMincoTraj(
    std::shared_ptr<plan_utils::SingulTrajData> display_traj) {
  ...
  for (unsigned int i = 0; i < display_traj->size(); ++i) {
    ...
    for (double t = 0; t <= total_duration; t += 0.01) {
      ...
      // std::cout<<"pos: "<<pt.transpose()<<" vel: "<<vel.transpose()<<" yaw:
      // "<<yaw<<std::endl;
  ...
  for (unsigned int i = 0; i < display_traj->size(); ++i) {
    ...
    for (double t = 0; t <= total_duration; t += 0.1) {
      ...
      offset1 =
          R * Eigen::Vector2d(car_length_ / 2.0 + car_d_cr_, car_width_ / 2.0);
      tmp1 = pos + offset1;
      point1.x = tmp1[0];
      point1.y = tmp1[1];
      point1.z = 0;
      ...
      offset3 = R * Eigen::Vector2d(-car_length_ / 2.0 + car_d_cr_,
                                    -car_width_ / 2.0);
      tmp3 = pos + offset3;
      point3.x = tmp3[0];
      point3.y = tmp3[1];
...
bool TrajPlanner::checkCollisionWithObs(const double& t_now) {
  ...
  double segment_i_start_time =
      traj_container_.singul_traj[segmentId].start_time;
  double segment_i_end_time = traj_container_.singul_traj[segmentId].end_time;
  int segment_i_singul =
      traj_container_.singul_traj[segmentId].traj.getDirection();
  if (t_now < segment_i_start_time || t_now > segment_i_end_time) {
    return collision;
  } else {
    double segment_i_duration = segment_i_end_time - segment_i_start_time;
    for (double t = t_now; t < segment_i_end_time - segment_i_duration / 3.0;
         t += 0.05) {
      Eigen::Vector2d pos = traj_container_.singul_traj[segmentId].traj.getPos(
          t - segment_i_start_time);
      Eigen::Vector2d vel =
          traj_container_.singul_traj[segmentId].traj.getdSigma(
              t - segment_i_start_time);
...
bool TrajPlanner::checkCollisionWithOtherCars(const double& t_now) {
  ...
  if (num_have_received_trajs < cars_num_) return collision;

  double t_start = traj_container_.singul_traj[0].start_time;
  double t_end =
      traj_container_.singul_traj[traj_container_.singul_traj.size() - 1]
          .end_time;
...
bool TrajPlanner::RunMINCOParking() {
  ...
  for (unsigned int i = 0; i < kino_trajs_.size(); i++) {
    ...
    for (int i = 0; i < piece_nums; i++) {
      ...
      for (int k = 0; k <= resolution; k++) {
        ...
        // cout << "t: " << t << endl;
        Eigen::Vector3d pos =
            kino_path_finder_->CalculateInitPos(t, kino_traj.singul);
    ...
    display_hPolys_.insert(display_hPolys_.end(), hPolys_.begin(),
                           hPolys_.end());
    auto corridor_t2 = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> corridor_ms =
        corridor_t2 - corridor_t1;
  ...
  int flag_success = ploy_traj_opt_->OptimizeTrajectory(
      iniState_container, finState_container, waypoints_container,
      duration_container, sfc_container, singul_container, worldtime, 0.0);
  ...
  if (flag_success) {
    ...
    for (unsigned int i = 0; i < kino_trajs_.size(); i++) {
      traj_container_.addSingulTraj(
          (*ploy_traj_opt_->getMinJerkOptPtr())[i].getTraj(singul_container[i]),
          worldtime, car_id_);  // todo time
      std::cout << "init duration: " << duration_container[i] << std::endl;
      std::cout << "pieceNum: " << waypoints_container[i].cols() + 1
                << std::endl;
      std::cout << "optimized total duration: "
                << (*ploy_traj_opt_->getMinJerkOptPtr())[i]
                       .getTraj(1)
                       .getTotalDuration()
                << std::endl;
      std::cout << "optimized jerk cost: "
                << (*ploy_traj_opt_->getMinJerkOptPtr())[i].getTrajJerkCost()
                << std::endl;
...
//         traj_container_.singul_traj[i].traj.getPositions(); Eigen::VectorXd
//         ts = traj_container_.singul_traj[i].traj.getDurations(); int
//         direction = traj_container_.singul_traj[i].traj.getDirection();
...
void TrajPlanner::getRectangleConst(std::vector<Eigen::Vector3d> statelist) {
  ...
  for (const auto state : statelist) {
    ...
    distance2center << car_width_ / 2.0, car_length_ / 2.0 + car_d_cr_,
        car_width_ / 2.0, car_length_ / 2.0 - car_d_cr_;
    ...
    while (have_stopped_expanding.norm() != 0) {
      for (int i = 0; i < 4; i++) {
        ...
        switch (i) {
          case 0:  // dy
            point1 = pos + ego_R * Eigen::Vector2d(-distance2center(3),
                                                   distance2center(0));
            point2 = pos + ego_R * Eigen::Vector2d(distance2center(1),
                                                   distance2center(0));
            newpoint1 =
                pos + ego_R * Eigen::Vector2d(-distance2center(3),
                                              distance2center(0) + step);
            newpoint2 =
                pos + ego_R * Eigen::Vector2d(distance2center(1),
                                              distance2center(0) + step);

            kino_path_finder_->checkCollisionUsingLine(point1, newpoint1,
                                                       isocc);
            if (isocc) {
            ...
            kino_path_finder_->checkCollisionUsingLine(newpoint1, newpoint2,
                                                       isocc);
            if (isocc) {
              have_stopped_expanding(i) = 0.0;
              break;
            ...
            kino_path_finder_->checkCollisionUsingLine(newpoint2, point2,
                                                       isocc);
            if (isocc) {
              have_stopped_expanding(i) = 0.0;
              break;
          ...
          case 1:  // dx
            point1 = pos + ego_R * Eigen::Vector2d(distance2center(1),
                                                   distance2center(0));
            point2 = pos + ego_R * Eigen::Vector2d(distance2center(1),
                                                   -distance2center(2));
            newpoint1 = pos + ego_R * Eigen::Vector2d(distance2center(1) + step,
                                                      distance2center(0));
            newpoint2 = pos + ego_R * Eigen::Vector2d(distance2center(1) + step,
                                                      -distance2center(2));
            ...
            distance2center(i) += step;
            if (distance2center(i) - car_length_ / 2.0 - car_d_cr_ >
                limitBound) {
              have_stopped_expanding(i) = 0.0;
              break;
            }
          ...
          case 2:  // -dy
            point1 = pos + ego_R * Eigen::Vector2d(distance2center(1),
                                                   -distance2center(2));
            point2 = pos + ego_R * Eigen::Vector2d(-distance2center(3),
                                                   -distance2center(2));
            newpoint1 =
                pos + ego_R * Eigen::Vector2d(distance2center(1),
                                              -distance2center(2) - step);
            newpoint2 =
                pos + ego_R * Eigen::Vector2d(-distance2center(3),
                                              -distance2center(2) - step);
          ...
          case 3:  // -dx
            ...
            point2 = pos + ego_R * Eigen::Vector2d(-distance2center(3),
                                                   distance2center(0));
            newpoint1 =
                pos + ego_R * Eigen::Vector2d(-distance2center(3) - step,
                                              -distance2center(2));
            newpoint2 =
                pos + ego_R * Eigen::Vector2d(-distance2center(3) - step,
                                              distance2center(0));
    ...
    point1 =
        pos + ego_R * Eigen::Vector2d(distance2center(1), distance2center(0));
    ...
    point2 =
        pos + ego_R * Eigen::Vector2d(distance2center(1), -distance2center(2));
    ...
    point3 =
        pos + ego_R * Eigen::Vector2d(-distance2center(3), -distance2center(2));
    ...
    point4 =
        pos + ego_R * Eigen::Vector2d(-distance2center(3), distance2center(0));
...
void TrajPlanner::setSwarmTrajs(const swarm_bridge::Trajectory& traj_msg) {
  ...
  kino_path_finder_->setAllCarsLastTrajs(swarm_last_traj_container_[car_id],
                                         car_id);
  ploy_traj_opt_->setAllCarsTrajs(surround_traj, car_id);
  ploy_traj_opt_->setAllCarsLastTrajs(swarm_last_traj_container_[car_id],
                                      car_id);
}
...
void TrajPlanner::setMapFree(double& time_now) {
  ...
  if (num_have_received_trajs < cars_num_) return;
  ...
  for (int car = 0; car < cars_num_; car++) {
    ...
    double i_th_segment_starttime =
        swarm_traj_container_[car].singul_traj[i_th_segment].start_time;
    double i_th_segment_endtime =
        swarm_traj_container_[car].singul_traj[i_th_segment].end_time;
    // if(time_now < i_th_segment_starttime || time_now > i_th_segment_endtime)
    //     return;
    if (time_now > i_th_segment_endtime) return;

    Eigen::Vector2d pos;
    double yaw;
    if (time_now < i_th_segment_starttime) {
      int i_th_last_segment =
          swarm_last_traj_container_[car].locateSingulId(time_now);
      double i_th_segment_last_start_time = swarm_last_traj_container_[car]
                                                .singul_traj[i_th_last_segment]
                                                .start_time;
      double i_th_segment_last_end_time = swarm_last_traj_container_[car]
                                              .singul_traj[i_th_last_segment]
                                              .end_time;
      if (time_now < i_th_segment_last_start_time ||
          time_now > i_th_segment_last_end_time)
        return;

      pos = swarm_last_traj_container_[car]
                .singul_traj[i_th_last_segment]
                .traj.getPos(time_now - i_th_segment_last_start_time);
      yaw = swarm_last_traj_container_[car]
                .singul_traj[i_th_last_segment]
                .traj.getAngle(time_now - i_th_segment_last_start_time);

      pos_vec.push_back(pos);
      yaw_vec.push_back(yaw);
    } else {
      pos = swarm_traj_container_[car].singul_traj[i_th_segment].traj.getPos(
          time_now - i_th_segment_starttime);
      yaw = swarm_traj_container_[car].singul_traj[i_th_segment].traj.getAngle(
          time_now - i_th_segment_starttime);
