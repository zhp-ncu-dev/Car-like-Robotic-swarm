...
void KinoAstar::setMap(MappingProcess::Ptr& ptr) { map_ptr_ = ptr; }
...
void KinoAstar::init(ros::NodeHandle& nh) {
  ...
  car_width_ -= 0.8;
  // expandNodesVis =
  // nh.advertise<sensor_msgs::PointCloud2>("/vis/expanded_nodes", 1);
}

void KinoAstar::setAllCarsTrajs(plan_utils::TrajContainer& trajectory,
                                int& car_id) {
...
void KinoAstar::setAllCarsLastTrajs(plan_utils::TrajContainer& trajectory,
                                    int& car_id) {
...
void KinoAstar::checkCollisionUsingPosAndYaw(const Eigen::Vector3d& state,
                                             bool& res) {
  res = false;
  ...
  for (int i = 0; i < 4; i++) {
    ...
    RayCaster raycaster;
    bool need_ray =
        raycaster.setInput(start_point / resolution_, end_point / resolution_);
...
void KinoAstar::checkCollisionUsingLine(const Eigen::Vector2d& start_pt,
                                        const Eigen::Vector2d& end_pt,
                                        bool& res) {
  res = false;
  RayCaster raycaster;
  bool need_ray =
      raycaster.setInput(start_pt / resolution_, end_pt / resolution_);
  // if(!need_ray)
  // {
  //     res = false;
  //     return;
...
void KinoAstar::setFreeSpaces(std::vector<Eigen::Vector2d>& pos_vec,
...
int KinoAstar::search(Eigen::Vector4d start_state, Eigen::Vector2d init_ctrl,
  ...
  if (!use3d)
    ...
    expanded_nodes_.insert(cur_node->index, yawToIndex(start_state[2]),
                           cur_node);
  ...
  while (!open_set_.empty()) {
    ...
    bool reach_horizon =
        (cur_node->state.head(2) - start_state_.head(2)).norm() >= horizon_;
    // double t1 = ros::Time::now().toSec();
    if ((cur_node->state.head(2) - end_state_.head(2)).norm() < 15.0 &&
        initsearch) {
    ...
    if (!initsearch) {
      if (start_state_[3] > 0) {
        for (double arc = resolution_; arc <= 2 * resolution_ + 1e-3;
             arc += resolution_) {
      ...
      for (double arc = -step_arc; arc <= step_arc + 1e-3;
           arc += 0.5 * step_arc) {
    ...
    for (auto& input : inputs) {
      ...
      tmp_g_score +=
          traj_steer_penalty * std::fabs(input[0]) * std::fabs(input[1]);
      tmp_g_score +=
          traj_steer_change_penalty * std::fabs(input[0] - cur_node->input[0]);
      tmp_g_score += cur_node->g_score;
      if (last_path_pos_.size() != 0) {
        if (cur_node->number < 20 &&
            cur_node->number + nearest_idx_ < last_path_pos_.size()) {
          Eigen::Vector2d last_nearest_pos =
              last_path_pos_[nearest_idx_ + cur_node->number];
          bool collision_between_two_nodes;
          checkCollisionUsingLine(pro_pos, last_nearest_pos,
                                  collision_between_two_nodes);
          if (collision_between_two_nodes &&
              map_ptr_->getVoxelState2d(last_nearest_pos) != 1) {
            // tmp_g_score += 100;
            continue;
          }
        }
      ...
      // std::cout<<"g1: "<<( ctrl_input(1)*ctrl_input(1) +
      // 10*ctrl_input(0)*ctrl_input(0) + w_time_) * tau<<" g0:
      // "<<backward_penalty<< "h: "<<lambda_heu_ * getHeu(pro_state,
      // end_state)<<"\n";
...
double KinoAstar::computeShotTraj(Eigen::Vector3d& state1,
...
Eigen::Vector3d KinoAstar::evaluatePos(double t) {
  ...
  double arclength =
      evaluateLength(CutTime, locallength, localtime, initv, finv);
  double tmparc = 0;
    ...
    if (tmparc >= arclength) {
      ...
      if (fabs(localTraj[i + 1][2] - localTraj[i][2]) >= M_PI) {
        ...
        if (localTraj[i + 1][2] <= 0) {
          normalize_yaw = l1 / l * localTraj[i][2] +
                          l2 / l * (localTraj[i + 1][2] + 2 * M_PI);
        } else if (localTraj[i][2] <= 0) {
          normalize_yaw = l1 / l * (localTraj[i][2] + 2 * M_PI) +
                          l2 / l * localTraj[i + 1][2];
        }
...
/*This function is used to get the truncated path list, which only includes the
 * points along the path*/
/*------"positionList_" is the main vector we are going to use in later
 * code-------------------------*/
...
// This function is used to find the sinularity point of the kinodynamic
// trajectory
/*"direction_change_idx_" is used in later code, which stores the index of the
 * singularity point*/
// attention: direction_change_idx_ not only includes the index of the direction
// change point, but also includes the indexes of the first point and the final
// point
void KinoAstar::getSingulNodes() {
  ...
  for (int i = 1; i < direction_change_idx_.size(); i++) {
    if (direction_change_idx_[i] - direction_change_idx_[i - 1] > 1) {
      ...
      double distance =
          (positionList_[singul_idx] - positionList_[pre_singul_idx]).norm();
      if (distance < checkl) {
...
bool KinoAstar::searchTime(plan_utils::KinoTrajData& flat_trajs,
  ...
  for (double dis = distance_resolution_; dis < total_s_;
       dis += distance_resolution_) {
    int locate_idx = 0;
    for (int i = 1; i <= end_index; i++) {
    ...
    double yaw_angle = atan2(start_direction * pos_direction(1),
                             start_direction * pos_direction(0));
    Eigen::Matrix2d Rot_mat;
    Rot_mat << cos(yaw_angle), -sin(yaw_angle), sin(yaw_angle), cos(yaw_angle);
  ...
  /*build s-t graph step2: enumurate every point of other car's trajectory and
   * check collision*/
  double collision_distance = car_width_;
  std::vector<Eigen::Vector3i> s_t_graph;
  int max_int_time = 0;
  for (int sur_id = 0; sur_id < cars_num_; sur_id++) {
    if (sur_id == car_id_ || !ifdynamic_) continue;

    int segments_num = swarm_traj_container_[sur_id].singul_traj.size();
    double traj_start_time =
        swarm_traj_container_[sur_id].singul_traj[0].start_time;
    double traj_end_time =
        swarm_traj_container_[sur_id].singul_traj[segments_num - 1].end_time;
    for (double world_t = start_world_time; world_t <= traj_end_time + 1e-3;
         world_t += time_resolution_) {
      ...
      Eigen::Matrix2d Rot_mat;
      Rot_mat << cos(surround_angle), -sin(surround_angle), sin(surround_angle),
          cos(surround_angle);
      ...
      for (int s_int = 0; s_int < discrete_positionList.size() - 1; s_int++) {
        if ((surround_p - discrete_positionList[s_int]).norm() <
            collision_distance) {
  ...
  start_node->s_t_v_idx =
      Eigen::Vector3i(t2idx(0.0), s2idx(0.0), v2idx(plan_start_vel));
  start_node->length = 0.0;
  start_node->distance2end = total_s_;
  start_node->acc = 0.0;
  start_node->g_score = 0.0;
  start_node->f_score =
      1 * calculateMiniTime2singulPoint(plan_start_vel, total_s_);
  ...
  while (!openSet_.empty()) {
    ...
    /*---------one shot, which needs the end velcity and distance to singul
     * point---------*/
    bool one_shot_success = true;
    double shot_duration = 0.0;
    std::vector<double> oneShot_trans_t;
    double d1 =
        (max_vel_ * max_vel_ - pro_start_vel * pro_start_vel) / (2 * max_input);
    ...
    if (pro_start2singuldistance >= d1 + d2) {
      ...
      oneShot_trans_t.at(0) = t1;
      oneShot_trans_t.at(1) = t1 + t2;
      oneShot_trans_t.at(2) = t1 + t2 + t3;
    }

    double d3 = pro_start_vel * pro_start_vel / (2 * max_input);
    if (pro_start2singuldistance >= d3 &&
        pro_start2singuldistance < (d1 + d2)) {
      double max_v = sqrt((2 * max_input * pro_start2singuldistance +
                           pro_start_vel * pro_start_vel) /
                          2);
    ...
    for (double shot_check_t = time_resolution_;
         shot_check_t <= shot_duration + 1e-3;
         shot_check_t += time_resolution_) {
      double cur_t, cur_s;
      cur_t = pro_start_t + shot_check_t;
      if (oneShot_trans_t.size() == 3) {
        if (shot_check_t < oneShot_trans_t.at(0)) {
          cur_s = pro_start_s + pro_start_vel * shot_check_t +
                  0.5 * max_input * shot_check_t * shot_check_t;
        } else if (shot_check_t >= oneShot_trans_t.at(0) &&
                   shot_check_t < oneShot_trans_t.at(1)) {
          double s1 = (max_vel_ * max_vel_ - pro_start_vel * pro_start_vel) /
                      (2 * max_input);
          ...
          double s1 = (max_vel_ * max_vel_ - pro_start_vel * pro_start_vel) /
                      (2 * max_input);
          double s2 =
              max_vel_ * (oneShot_trans_t.at(1) - oneShot_trans_t.at(0));
          double s3 =
              max_vel_ * (shot_check_t - oneShot_trans_t.at(1)) -
              0.5 * max_input * pow(shot_check_t - oneShot_trans_t.at(1), 2);
        ...
        if (shot_check_t < oneShot_trans_t.at(0)) {
          ...
          double s1 = pro_start_vel * oneShot_trans_t.at(0) +
                      0.5 * max_input * pow(oneShot_trans_t.at(0), 2);
          double s2 =
              max_v * (shot_check_t - oneShot_trans_t.at(0)) -
              0.5 * max_input * pow(shot_check_t - oneShot_trans_t.at(0), 2);
          cur_s = pro_start_s + s1 + s2;
        }
      }
    ...
    if (one_shot_success) {
      ...
      cout << "The number of expanded nodes: " << debug_middle_nodes.size()
           << endl;
      ...
      reverse(middle_nodes_.begin(),
              middle_nodes_
                  .end());  // middle_nodes_ include the first no distance node

      flat_trajs.clear();

      double segment_duration = middle_nodes_.back()->end_t + shot_duration;
      double sampletime = 0.1;
      if (sampletime > segment_duration) sampletime = segment_duration / 2.0;

      std::vector<Eigen::Vector3d> traj_pts;
      std::vector<double> thetas;
      double samplet;
      for (samplet = sampletime; samplet < segment_duration;
           samplet += sampletime) {
        ...
        traj_pt << traj_pt_pos, sampletime;
      ...
      double segment_end_yaw = atan2(start_direction * last_direction(1),
                                     start_direction * last_direction(0));
      double segment_end_vel = start_direction > 0 ? non_siguav : -non_siguav;
      traj_pts.push_back(Eigen::Vector3d(last_pos(0), last_pos(1),
                                         segment_duration - samplet));
    ...
    double res = 1.0;
    for (double input = -max_input; input <= max_input + 1e-3;
         input += res * max_input) {
    ...
    // for(double tau = 2 * time_resolution_; tau <= 3 * 2 * time_resolution_ +
    // 1e-3; tau += 2 * time_resolution_)
    // {
    ...
    for (int i = 0; i < ctrl_inputs.size(); i++) {
      for (int j = 0; j < durations.size(); j++) {
        ...
        double pro_end_s = pro_start_s + pro_start_vel * duration +
        ...
        Eigen::Vector3i pro_end_stv_idx(t2idx(pro_end_t), s2idx(pro_end_s),
                                        v2idx(pro_end_vel));

        if (pro_end_vel > max_vel_ || pro_end_vel < non_siguav) continue;
        if (pro_end_vel * pro_end_vel / (2 * max_input) >
            (pro_end2singuldistance - 1e-3))
          continue;
        if (pro_end_s > total_s_ || pro_end_s < pro_start_s) continue;
        ...
        for (double check_t = pro_end_t - pro_start_t; check_t >= -1e-3;
             check_t -= time_resolution_) {
          double check_s = pro_start_s + pro_start_vel * check_t +
                           0.5 * input * check_t * check_t;
          if (check_s > total_s_ || check_s < pro_start_s) {
            collision = true;
            break;
          }
          // no need to check velocity, only check end velocity is enough. But
          // it's necessary to check s. if t is beyond the max t, then no need
          // to check any more, because there will not be collision
          int check_int_t = t2idx(check_t + pro_start_t);
          if (check_int_t >= max_int_t) continue;
        ...
        double tmp_h_score = 1 * calculateMiniTime2singulPoint(
                                     pro_end_vel, pro_end2singuldistance);
        ...
        for (int k = 0; k < expanded_nodes_from_one_node.size(); k++) {
          ...
          if (pro_end_stv_idx == node_from_cur_node->s_t_v_idx) {
            ...
            if (node_from_cur_node->f_score > tmp_f_score) {
              ...
              node_from_cur_node->length =
                  pro_start_vel * duration + 0.5 * input * duration * duration;
...
double KinoAstar::calculateMiniTime2singulPoint(double initspeed,
                                                double distance2end) {
  double d1 = (max_vel_ * max_vel_ - initspeed * initspeed) / (2 * max_acc_) +
              max_vel_ * max_vel_ / (2 * max_acc_);
  ...
  if (distance2end >= d3) {
    double vmax =
        sqrt((2 * max_acc_ * distance2end + initspeed * initspeed) / 2.0);
...
int KinoAstar::t2idx(double t) { return floor((t + 1e-3) / time_resolution_); }

int KinoAstar::v2idx(double v) { return floor(v / velocity_resolution_); }
...
Eigen::Vector3d KinoAstar::CalculateInitPos(double& t, int& singul) {
  ...
  if (t_bar < shot_start_t - 1e-3) {
    for (MiddleNodePtr middle_node : middle_nodes_) {
      if (t_bar >= (middle_node->start_t - 1e-5) &&
          t_bar < middle_node->end_t) {
        double node_t_bar = t_bar - middle_node->start_t;
        double node_start_v = middle_node->start_vel;
        double node_acc = middle_node->acc;
        cur_s = middle_node->start_s + node_start_v * node_t_bar +
                0.5 * node_acc * pow(node_t_bar, 2);
        break;
      }
    }
  } else {
    ...
    double d1 =
        (max_vel_ * max_vel_ - shot_start_v * shot_start_v) / (2 * max_acc_);
    ...
    if (shot_length > (d1 + d2)) {
      ...
      double t3 = max_vel_ / max_acc_;
      if (shot_t_bar < t1) {
        cur_s = shot_start_s + shot_start_v * shot_t_bar +
                0.5 * max_acc_ * pow(shot_t_bar, 2);
      } else if (shot_t_bar >= t1 && shot_t_bar < (t1 + t2)) {
        cur_s = shot_start_s + d1 + max_vel_ * (shot_t_bar - t1);
      } else {
        cur_s = shot_start_s + d1 + max_vel_ * t2 +
                max_vel_ * (shot_t_bar - t1 - t2) -
                0.5 * max_acc_ * pow(shot_t_bar - t1 - t2, 2);
      }
    } else if (shot_length <= d1 + d2 && shot_length > (d3 - 1e-3)) {
      double max_v =
          sqrt((2 * max_acc_ * shot_length + shot_start_v * shot_start_v) / 2);
      double t1 = (max_v - shot_start_v) / max_acc_;
      double t2 = max_v / max_acc_;
      if (shot_t_bar < t1) {
        cur_s = shot_start_s + shot_start_v * shot_t_bar +
                0.5 * max_acc_ * shot_t_bar * shot_t_bar;
      } else {
        cur_s = shot_start_s +
                (max_v * max_v - shot_start_v * shot_start_v) / (2 * max_acc_) +
                max_v * (shot_t_bar - t1) -
                0.5 * max_acc_ * pow(shot_t_bar - t1, 2);
      }
    } else {
      // ROS_ERROR("Wrong! Cannot calculate the init pos!");
      double max_input = shot_start_v * shot_start_v / (2 * shot_length);
      cur_s = shot_start_s + shot_start_v * shot_t_bar -
              0.5 * max_input * pow(shot_t_bar, 2);
    }
  }
  ...
  for (int i = 0; i < positionList_.size() - 1; i++) {
    double cur_path_distance =
        (positionList_.at(i + 1) - positionList_.at(i)).norm();
    if (cur_s >= path_distance - 1e-3 &&
        cur_s < path_distance + cur_path_distance) {
  ...
  Eigen::Vector2d sample_pos =
      path_node_start_pos + s_bar / path_node_dis * path_node_direction;
  double sample_yaw =
      atan2(singul * path_node_direction(1), singul * path_node_direction(0));

  return Eigen::Vector3d(sample_pos(0), sample_pos(1),
                         normalize_angle(sample_yaw));
}

// This function returns the distance between current node's "start" to the
// nearest singul point
  ...
  for (int i = 0; i < direction_change_idx_.size() - 1; i++) {
    if (idx > direction_change_idx_[i] && idx < direction_change_idx_[i + 1]) {
      ...
      for (int j = idx - 1; j < direction_change_idx_[i + 1]; j++) {
        length_to_nearest_singul_point +=
            (positionList_[j + 1] - positionList_[j]).norm();
      }
...
void KinoAstar::getKinoNode(plan_utils::KinoTrajData& flat_trajs) {
  ...
  double tmp_len = 0;
  int truncate_idx = 0;
  for (truncate_idx = 0; truncate_idx < roughSampleList.size() - 1;
       truncate_idx++) {
    tmp_len +=
        (roughSampleList[truncate_idx + 1] - roughSampleList[truncate_idx])
            .norm();
    if (tmp_len > truncate_len) {
      break;
    }
  }
  roughSampleList.assign(roughSampleList.begin(),
                         roughSampleList.begin() + truncate_idx + 1);
  ...
  int lastS = (SampleTraj[1] - SampleTraj[0])
                          .head(2)
                          .dot(Eigen::Vector2d(cos(SampleTraj[0][2]),
                                               sin(SampleTraj[0][2]))) >= 0
                  ? 1
                  : -1;
  ...
  for (int i = 0; i < SampleTraj.size() - 1; i++) {
    ...
    int curS =
        (state2 - state1)
                    .head(2)
                    .dot(Eigen::Vector2d(cos(state1[2]), sin(state1[2]))) >= 0
            ? 1
            : -1;
    if (curS * lastS >= 0) {
  ...
  if (shot_timeList.size() >= 2) {
    shot_timeList[0] =
        evaluateDuration(shot_lengthList[0], startvel, non_siguav);
    shot_timeList[shot_timeList.size() - 1] =
        evaluateDuration(shot_lengthList.back(), non_siguav, endvel);
  } else {
  ...
  for (int i = 0; i < shot_lengthList.size(); i++) {
    ...
    std::vector<Eigen::Vector3d> localTraj;
    localTraj.assign(SampleTraj.begin() + shotindex[i],
                     SampleTraj.begin() + shotindex[i + 1] + 1);
    traj_pts.clear();
    thetas.clear();
    double samplet;
    double tmparc = 0;
    ...
    for (samplet = sampletime; samplet < shot_timeList[i];
         samplet += sampletime) {
      double arc =
          evaluateLength(samplet, locallength, shot_timeList[i], initv, finv);
      // find the nearest point
        ...
        if (tmparc >= arc) {
          ...
          if (fabs(localTraj[k + 1][2] - localTraj[k][2]) >= M_PI) {
            ...
            if (localTraj[k + 1][2] <= 0) {
              normalize_yaw = l1 / l * localTraj[k][2] +
                              l2 / l * (localTraj[k + 1][2] + 2 * M_PI);
            } else if (localTraj[k][2] <= 0) {
              normalize_yaw = l1 / l * (localTraj[k][2] + 2 * M_PI) +
                              l2 / l * localTraj[k + 1][2];
            }
            yaw = normalize_yaw;
          }
    ...
    traj_pts.push_back(
        Eigen::Vector3d(localTraj.back()[0], localTraj.back()[1],
...
double KinoAstar::evaluateDuration(double length, double startV, double endV) {
  double critical_len;  // the critical length of two-order optimal control, acc
                        // is the input
  if (startV > max_vel_ || endV > max_vel_) {
    ROS_ERROR(
        "kinoAstar:evaluateDuration:start or end vel is larger that the "
        "limit!");
  }
  ...
  critical_len =
      (maxv2 - startv2) / (2 * max_acc_) + (maxv2 - endv2) / (2 * max_acc_);
...
double KinoAstar::evaluateLength(double curt, double locallength,
                                 double localtime, double startV, double endV) {
  double critical_len;  // the critical length of two-order optimal control, acc
                        // is the input
  if (startV > max_vel_ || endV > max_vel_) {
    ROS_ERROR(
        "kinoAstar:evaluateLength:start or end vel is larger that the limit!");
  }
  double startv2 = pow(startV, 2);
  double endv2 = pow(endV, 2);
  double maxv2 = pow(max_vel_, 2);
  ...
  if (locallength >= critical_len) {
    ...
    if (curt <= tmpt) {
      ...
      return startV * tmpt + 0.5 * max_acc_ * pow(tmpt, 2) +
             tmpv * (curt - tmpt) - 0.5 * max_acc_ * pow(curt - tmpt, 2);
    }
  }
}
...
void KinoAstar::getFlatState(Eigen::Vector4d state,
  ...
  flat_state << state.head(2), init_R * Eigen::Vector2d(vel, 0.0),
      init_R * Eigen::Vector2d(control_input(1), std::tan(control_input(0)) /
                                                     car_wheelbase_ *
                                                     std::pow(vel, 2));
}
