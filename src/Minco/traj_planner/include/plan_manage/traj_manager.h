...
class TrajPlanner {
 public:
  ...
  void setInitStateAndInput(const Eigen::Vector4d& state,
                            const double& start_time);
  void setInitStateAndInput(const double& t,
                            Eigen::Vector4d& replan_init_state);
  void setSwarmTrajs(const swarm_bridge::Trajectory& traj_msg);
  void setMapFree(double& time_now);

  std::vector<Eigen::MatrixXd> display_hPolys() const {
    return display_hPolys_;
  }
  // plan_utils::SurroundTrajData display_surround_trajs() const {return
  // surround_trajs;};
  ...
  //   // const plan_utils::SingulTrajData* const singul_traj =
  //   &traj_container_.singul_traj; return &traj_container_.singul_traj;
  // }
