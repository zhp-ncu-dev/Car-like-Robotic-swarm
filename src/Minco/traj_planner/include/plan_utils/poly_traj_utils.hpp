...
class Piece  // component from poly
{
 ...
 public:
  ...
  //   double c_x = position(0) + vp_.d_cr() * cos_theta; // the vehicle model
  //   is based on the rear wheel double c_y = position(1) + vp_.d_cr() *
  //   sin_theta; double d_wx = vp_.width() / 2 * sin_theta; double d_wy =
  //   vp_.width() / 2 * cos_theta; double d_lx = vp_.length() / 2 * cos_theta;
  //   double d_ly = vp_.length() / 2 * sin_theta;
  //   // Counterclockwise from left-front vertex
  //   BoundVertices.push_back(Eigen::Vector2d(c_x - d_wx + d_lx, c_y + d_wy +
  //   d_ly)); BoundVertices.push_back(Eigen::Vector2d(c_x - d_wx - d_lx, c_y -
  //   d_ly + d_wy)); BoundVertices.push_back(Eigen::Vector2d(c_x + d_wx - d_lx,
  //   c_y - d_wy - d_ly)); BoundVertices.push_back(Eigen::Vector2d(c_x + d_wx +
  //   d_lx, c_y + d_ly - d_wy));
  ...
  //       // curv
  //       otherstate[1] =
  //       (dsigma(0)*ddsigma(1)-dsigma(1)*ddsigma(0))/std::pow(otherstate[2],
  //       3);
  //       // acc
  //       otherstate[3] =
...
class Trajectory {
 ...
 public:
  ...
  //     // std::cout << "state->vec_position is:"<< state->vec_position <<
  //     std::endl;
  ...
  //     // std::cout << "state->acceleration is: "<< otherstate(3) <<
  //     std::endl;
