...
class NodeHashTable {
 ...
 public:
  ...
  void insert(Eigen::Vector2i idx, int yaw_idx, NodePtr node) {
    data_3d_.insert(
        std::make_pair(Eigen::Vector3i(idx(0), idx(1), yaw_idx), node));
  }
...
class MiddleNodeHashTable {
 private:
  /* data */
  std::unordered_map<Eigen::Vector3i, MiddleNodePtr,
...
class KinoAstar {
 private:
  ...
  double evaluateLength(double curt, double locallength, double localtime,
                        double startV = 0.0, double endV = 0.0);
  double evaluateDuration(double length, double startV = 0.0,
                          double endV = 0.0);
 ...
 public:
  ...
  void setFreeSpaces(std::vector<Eigen::Vector2d>& pos_vec,
  ...
  void checkCollisionUsingLine(const Eigen::Vector2d& start_pt,
                               const Eigen::Vector2d& end_pt, bool& res);
