 ...
 * It works only with containers based on double and sequential memory
 * allocation.
 */
...
struct mkl_operations {
  ...
  struct scale_sum2 {
    ...
    void operator()(T1 &t1, const T2 &t2, const T3 &t3)
        const {  // t1 = m_alpha1 * t2 + m_alpha2 * t3;
  ...
  struct scale_sum3 {
    ...
    void operator()(T1 &t1, const T2 &t2, const T3 &t3, const T4 &t4)
        const {  // t1 = m_alpha1 * t2 + m_alpha2 * t3 + m_alpha3 * t4;
      // we get Containers that have size() and [i]-access
  ...
  struct scale_sum4 {
    ...
    scale_sum4(const Fac1 alpha1, const Fac2 alpha2, const Fac3 alpha3,
               const Fac4 alpha4)
        : m_alpha1(alpha1),
          m_alpha2(alpha2),
          m_alpha3(alpha3),
    ...
    void operator()(T1 &t1, const T2 &t2, const T3 &t3, const T4 &t4,
                    const T5 &t5) const {  // t1 = m_alpha1 * t2 + m_alpha2 * t3
                                           // + m_alpha3 * t4 + m_alpha4 * t5;
      // we get Containers that have size() and [i]-access
  ...
  struct scale_sum5 {
    ...
    void operator()(
        ...
        const T6 &t6) const {  // t1 = m_alpha1 * t2 + m_alpha2 * t3 + m_alpha3
                               // * t4 + m_alpha4 * t5 + m_alpha5 * t6;
      // we get Containers that have size() and [i]-access
