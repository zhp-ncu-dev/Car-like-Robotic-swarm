 ...
 public:
  ...
  value_type error(const state_type &x, const state_type &xold,
    ...
    for (size_t i = 0; i < n; ++i) {
      sk = m_atol + m_rtol * max BOOST_PREVENT_MACRO_SUBSTITUTION(abs(xold[i]),
                                                                  abs(x[i]));
  ...
  value_type last_error(void) const { return m_err_old; }

  template <class System>
  boost::numeric::odeint::controlled_step_result try_step(System sys,
                                                          state_type &x,
                                                          time_type &t,
                                                          time_type &dt) {
    m_xnew_resizer.adjust_size(
        x, detail::bind(&controller_type::template resize_m_xnew<state_type>,
  ...
  boost::numeric::odeint::controlled_step_result try_step(System sys,
    ...
    m_xerr_resizer.adjust_size(
        x, detail::bind(&controller_type::template resize_m_xerr<state_type>,
                        detail::ref(*this), detail::_1));
    ...
    value_type fac = max BOOST_PREVENT_MACRO_SUBSTITUTION(
        fac2,
        min BOOST_PREVENT_MACRO_SUBSTITUTION(fac1, pow(err, 0.25) / safe));
    ...
    if (err <= 1.0) {
      if (m_first_step) {
        ...
        value_type fac_pred =
            (m_dt_old / dt) * pow(err * err / m_err_old, 0.25) / safe;
