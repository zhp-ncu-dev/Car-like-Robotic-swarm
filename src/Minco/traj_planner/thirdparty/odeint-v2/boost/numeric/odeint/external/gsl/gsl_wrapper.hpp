...
class gsl_vector_iterator
 ...
 public:
  ...
  explicit gsl_vector_iterator(gsl_vector *p)
      : m_p(p->data), m_stride(p->stride) {}
 ...
 */
class const_gsl_vector_iterator
    : public boost::iterator_facade<const_gsl_vector_iterator, const double,
                                    boost::random_access_traversal_tag> {
 public:
  const_gsl_vector_iterator(void) : m_p(0), m_stride(0) {}
  explicit const_gsl_vector_iterator(const gsl_vector *p)
      : m_p(p->data), m_stride(p->stride) {}
  const_gsl_vector_iterator(const gsl_vector_iterator &p)
      : m_p(p.m_p), m_stride(p.m_stride) {}
...
// template<>
inline gsl_vector_iterator range_end(gsl_vector *x) { return end_iterator(x); }
...
struct state_wrapper<gsl_vector *> {
  ...
  state_wrapper() { m_v = gsl_vector_alloc(1); }
  ...
  ~state_wrapper() { gsl_vector_free(m_v); }
};
