...
template <class T, class L, class A, class T2, class L2, class A2>
struct resize_impl<boost::numeric::ublas::matrix<T, L, A>,
                   boost::numeric::ublas::matrix<T2, L2, A2> > {
...
struct same_size_impl<boost::numeric::ublas::permutation_matrix<T, A>,
  ...
  static bool same_size(
      const boost::numeric::ublas::permutation_matrix<T, A> &m,
      const boost::numeric::ublas::vector<T_V, A_V> &v) {
...
struct state_wrapper<
    boost::numeric::ublas::permutation_matrix<T, A> >  // with resizing
{
  ...
  state_wrapper()
      : m_v(1)  // permutation matrix constructor requires a size, choose 1 as
                // default
  {}
};
...
// namespace odeint {
//
///* specialization for permutation matrices wrapper because we need to change
///the construction */
// template< class T , class A >
...
//    state_type m_v;
//
//    state_wrapper() : m_v( 1 ) // permutation matrix constructor requires a
//    size, choose 1 as default { }
//
...
/*
 * preparing ublas::matrix for boost::range, such that ublas::matrix can be used
 * in all steppers with the range algebra
 */

namespace boost {
template <class T, class L, class A>
struct range_mutable_iterator<boost::numeric::ublas::matrix<T, L, A> > {
  typedef typename boost::numeric::ublas::matrix<T, L, A>::array_type::iterator
      type;
};
...
struct range_const_iterator<boost::numeric::ublas::matrix<T, L, A> > {
  typedef typename boost::numeric::ublas::matrix<
      T, L, A>::array_type::const_iterator type;
};
...
template <class T, class L, class A>
inline typename matrix<T, L, A>::array_type::iterator range_begin(
    matrix<T, L, A> &x) {
  return x.data().begin();
}

template <class T, class L, class A>
inline typename matrix<T, L, A>::array_type::const_iterator range_begin(
    const matrix<T, L, A> &x) {
  return x.data().begin();
}

template <class T, class L, class A>
inline typename matrix<T, L, A>::array_type::iterator range_end(
    matrix<T, L, A> &x) {
  return x.data().end();
}

template <class T, class L, class A>
inline typename matrix<T, L, A>::array_type::const_iterator range_end(
    const matrix<T, L, A> &x) {
  return x.data().end();
}
