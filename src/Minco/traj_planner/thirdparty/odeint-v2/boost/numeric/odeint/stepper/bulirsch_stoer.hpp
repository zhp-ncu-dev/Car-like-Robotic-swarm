/*
  ...
  Springer Series in Comput. Mathematics, Vol. 8, Springer-Verlag 1987, Second
  revised edition 1993. [end_description]
...
class bulirsch_stoer {
 public:
  ...
  typedef bulirsch_stoer<State, Value, Deriv, Time, Algebra, Operations,
                         Resizer>
  ...
  bulirsch_stoer(value_type eps_abs = 1E-6, value_type eps_rel = 1E-6,
    ...
    for (unsigned short i = 0; i < m_k_max + 1; i++) {
      ...
      static_cast<value_type>( 1 ) , min BOOST_PREVENT_MACRO_SUBSTITUTION(
      static_cast<value_type>( m_k_max-1 ) , logfact ));
      */
    }
  }
  ...
  /**
   * \brief Second version to solve the forwarding problem, can be used with
  ...
  controlled_step_result try_step(System system, StateInOut &x,
    ...
    m_xnew_resizer.adjust_size(
        x, detail::bind(
               &controlled_error_bs_type::template resize_m_xnew<StateInOut>,
  ...
  controlled_step_result try_step(System system, const StateIn &in,
    ...
    if (m_resizer.adjust_size(
            in, detail::bind(
                    &controlled_error_bs_type::template resize_impl<StateIn>,
                    detail::ref(*this), detail::_1))) {
        ...
        if ((k == m_current_k_opt - 1) ||
          ...
          if (error < 1.0) {
            ...
            if ((work[k] < KFAC2 * work[k - 1]) || (m_current_k_opt <= 2)) {
              ...
              new_h = h_opt[k];
              new_h *= static_cast<value_type>(m_cost[k + 1]) /
    ...
    if (!m_last_step_rejected ||
        boost::numeric::odeint::detail::less_with_sign(new_h, dt, dt)) {
      m_dt_last = new_h;
      dt = new_h;
    }
 ...
 private:
  ...
  controlled_step_result try_step_v1(System system, StateInOut &x, time_type &t,
    ...
    m_dxdt_resizer.adjust_size(
        x, detail::bind(
               &controlled_error_bs_type::template resize_m_dxdt<StateInOut>,
               detail::ref(*this), detail::_1));
    sys(x, m_dxdt.m_v, t);
  ...
  void extrapolate(size_t k, state_table_type &table, const value_matrix &coeff,
                   StateInOut &xest)
  ...
  */
  ...
  controlled_step_result set_k_opt(size_t k, const inv_time_vector &work,
                                   const time_vector &h_opt, time_type &dt)
  /* calculates the optimal stage number */
  {
    ...
    if ((work[k - 1] < KFAC1 * work[k]) || (k == m_k_max)) {  // order decrease
      ...
      return success;
               ...
               (k == m_k_max - 1)) {  // same order - also do this if last step
                                      // got rejected
      m_current_k_opt = k;
      dt = h_opt[m_current_k_opt];
      return success;
  ...
  bool should_reject(value_type error, size_t k) const {
    if (k == m_current_k_opt - 1) {
      ...
      const value_type d =
          m_interval_sequence[m_current_k_opt] / m_interval_sequence[0];
      return (error > d * d);
    } else
      return error > 1.0;
  }
  ...
  default_error_checker<value_type, algebra_type, operations_type>
      m_error_checker;
  modified_midpoint<state_type, value_type, deriv_type, time_type, algebra_type,
...
/**
 * \fn bulirsch_stoer::bulirsch_stoer( value_type eps_abs , value_type eps_rel ,
 * value_type factor_x , value_type factor_dxdt ) \brief Constructs the
 * bulirsch_stoer class, including initialization of the error bounds.
 *
...
/**
 * \fn bulirsch_stoer::try_step( System system , StateInOut &x , time_type &t ,
...
/**
 * \fn bulirsch_stoer::try_step( System system , StateInOut &x , const DerivIn
 * &dxdt , time_type &t , time_type &dt ) \brief Tries to perform one step.
 *
 * This method tries to do one step with step size dt. If the error estimate
 * is to large, the step is rejected and the method returns fail and the
 * step size dt is reduced. If the error estimate is acceptably small, the
...
/**
 * \fn bulirsch_stoer::try_step( System system , const StateIn &in , time_type
 * &t , StateOut &out , time_type &dt ) \brief Tries to perform one step.
 *
...
/**
 * \fn bulirsch_stoer::try_step( System system , const StateIn &in , const
 * DerivIn &dxdt , time_type &t , StateOut &out , time_type &dt ) \brief Tries
 * to perform one step.
 *
 * This method tries to do one step with step size dt. If the error estimate
 * is to large, the step is rejected and the method returns fail and the
