...
size_t integrate_times(Stepper stepper, System system, State &start_state,
  ...
  while (true) {
    ...
    while (less_with_sign(current_time, *start_time, current_dt)) {
      current_dt =
          min BOOST_PREVENT_MACRO_SUBSTITUTION(dt, *start_time - current_time);
...
size_t integrate_times(Stepper stepper, System system, State &start_state,
  ...
  while (start_time != end_time) {
    while ((start_time != end_time) &&
           less_eq_with_sign(*start_time, stepper.current_time(),
    ...
    if (less_eq_with_sign(stepper.current_time() + stepper.current_time_step(),
                          last_time_point, stepper.current_time_step())) {
      stepper.do_step(system);
      ++count;
    } else if (start_time !=
               end_time) {  // do the last step ending exactly on the end point
