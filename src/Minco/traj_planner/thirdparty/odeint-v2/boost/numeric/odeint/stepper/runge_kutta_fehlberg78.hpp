...
struct rk78_coefficients_db : boost::array<Value, 13> {
  rk78_coefficients_db(void) {
    (*this)[0] = static_cast<Value>(0) -
                 static_cast<Value>(41) / static_cast<Value>(840);
    (*this)[1] = static_cast<Value>(0);
    (*this)[2] = static_cast<Value>(0);
    (*this)[3] = static_cast<Value>(0);
    (*this)[4] = static_cast<Value>(0);
...
class runge_kutta_fehlberg78
    : public explicit_error_generic_rk<13, 8, 8, 7, State, Value, Deriv, Time,
  ...
  runge_kutta_fehlberg78(const algebra_type &algebra = algebra_type())
      : stepper_base_type(
            boost::fusion::make_vector(
                rk78_coefficients_a1<Value>(), rk78_coefficients_a2<Value>(),
            ...
            rk78_coefficients_b<Value>(), rk78_coefficients_db<Value>(),
            rk78_coefficients_c<Value>(), algebra) {}
};
 ...
 *
 * The Runge-Kutta Fehlberg 78 method is a standard method for high-precision
 * applications. The method is explicit and fulfills the Error Stepper concept.
 * Step size control is provided but continuous output is not available for this
 * method.
 *
 * This class derives from explicit_error_stepper_base and inherits its
 * interface via CRTP (current recurring template pattern). Furthermore, it
 * derivs from explicit_error_generic_rk which is a generic Runge-Kutta
 * algorithm with error estimation. For more details see
 * explicit_error_stepper_base and explicit_error_generic_rk.
 *
...
/**
 * \fn runge_kutta_fehlberg78::runge_kutta_fehlberg78( const algebra_type
 * &algebra ) \brief Constructs the runge_kutta_cash_fehlberg78 class. This
 * constructor can be used as a default constructor if the algebra has a default
 * constructor. \param algebra A copy of algebra is made and stored inside
