/*
 ...
 [begin_description]
 Implementation of the Dense-output stepper for all steppers. Note, that this
 class does not computes the result but serves as an interface.
 [end_description]
...
class dense_output_runge_kutta<Stepper, stepper_tag> {
 public:
  ...
  void calc_state(time_type t, StateOut& x) const {
    m_stepper.calc_state(x, t, get_old_state(), m_t_old, get_current_state(),
                         m_t);
  }

  /**
   * \brief Calculates the solution at an intermediate point. Solves the
   * forwarding problem \param t The time at which the solution should be
   * calculated, has to be in the current time interval. \param x The output
   * variable where the result is written into, can be a boost range.
   */
  template <class StateOut>
   ...
   */
  time_type previous_time(void) const { return m_t_old; }
...
/**
 * \brief The class representing dense-output Runge-Kutta steppers with FSAL
 * property.
 *
 * The interface is the same as for dense_output_runge_kutta< Stepper ,
 * stepper_tag >. This class provides dense output functionality based on
 * methods with step size controlled
 *
 *
 * \tparam Stepper The stepper type of the underlying algorithm.
 */
...
class dense_output_runge_kutta<Stepper, explicit_controlled_stepper_fsal_tag> {
 public:
  ...
  void initialize(const StateType& x0, time_type t0, time_type dt0) {
    m_resizer.adjust_size(
        x0, detail::bind(&dense_output_stepper_type::template resize<StateType>,
                         detail::ref(*this), detail::_1));
    boost::numeric::odeint::copy(x0, get_current_state());
    m_t = t0;
    m_dt = dt0;
  ...
  std::pair<time_type, time_type> do_step(System system) {
    ...
    size_t count = 0;
    do {
      res = m_stepper.try_step(system, get_current_state(), get_current_deriv(),
                               m_t, get_old_state(), get_old_deriv(), m_dt);
      if (count++ == max_count)
        throw std::overflow_error(
            "dense_output_controlled_explicit : too much iterations!");
    } while (res == fail);
    toggle_current_state();
  ...
  template <class StateOut>
  void calc_state(time_type t, StateOut& x) const {
    m_stepper.stepper().calc_state(t, x, get_old_state(), get_old_deriv(),
                                   m_t_old, get_current_state(),
  ...
  bool resize(const StateIn& x) {
    ...
    resized |= adjust_size_by_resizeability(
        m_x2, x, typename is_resizeable<state_type>::type());
    resized |= adjust_size_by_resizeability(
        m_dxdt1, x, typename is_resizeable<deriv_type>::type());
    resized |= adjust_size_by_resizeability(
        m_dxdt2, x, typename is_resizeable<deriv_type>::type());
    return resized;
  }
