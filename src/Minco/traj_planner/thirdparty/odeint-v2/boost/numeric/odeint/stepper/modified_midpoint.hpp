...
class modified_midpoint
    : public explicit_stepper_base<
          modified_midpoint<State, Value, Deriv, Time, Algebra, Operations,
                            Resizer>,
...
#endif
{
 ...
 public:
  typedef explicit_stepper_base<modified_midpoint<State, Value, Deriv, Time,
                                                  Algebra, Operations, Resizer>,
                                2, State, Value, Deriv, Time, Algebra,
                                Operations, Resizer>
      stepper_base_type;
  ...
  void do_step_impl(System system, const StateIn &in, const DerivIn &dxdt,
    ...
    while (i != m_steps) {
      ...
      stepper_base_type::m_algebra.for_each3(
          ...
          typename operations_type::template scale_sum_swap2<value_type,
                                                             time_type>(val1,
                                                                        h2));
      th += h;
      sys(m_x1.m_v, m_dxdt.m_v, th);
    ...
    stepper_base_type::m_algebra.for_each4(
        ...
        typename operations_type::template scale_sum3<value_type, value_type,
                                                      time_type>(val05, val05,
                                                                 val05 * h));
  }

  void set_steps(unsigned short steps) { m_steps = steps; }

  unsigned short steps(void) const { return m_steps; }
 ...
 private:
  ...
  bool resize_impl(const StateIn &x) {
    ...
    resized |= adjust_size_by_resizeability(
        m_x0, x, typename is_resizeable<state_type>::type());
    resized |= adjust_size_by_resizeability(
...
/* Modified midpoint which stores derivatives and state at dt/2 in some external
 * storage for later usage in dense output calculation This Stepper is for use
 * in Bulirsch Stoer only. It DOES NOT meet any stepper concept.
 */
template <class State, class Value = double, class Deriv = State,
          class Time = Value, class Algebra = range_algebra,
          class Operations = default_operations,
...
class modified_midpoint_dense_out {
 public:
  ...
  typedef modified_midpoint_dense_out<State, Value, Deriv, Time, Algebra,
                                      Operations, Resizer>
      stepper_type;
   ...
   * stores approximation for x(t+dt/2) in x_mp and all evaluated function
   * results in derivs
   *
   */
  ...
  void do_step(System system, const StateIn &in, const DerivIn &dxdt,
    ...
    m_resizer.adjust_size(in,
                          detail::bind(&stepper_type::template resize<StateIn>,
                                       detail::ref(*this), detail::_1));
    ...
    if (m_steps == 2)
      // result of first step already gives approximation at the center of the
      // interval
    ...
    while (i != m_steps) {
      ...
      m_algebra.for_each3(m_x1.m_v, m_x0.m_v, derivs[i - 1].m_v,
                          typename operations_type::template scale_sum_swap2<
                              value_type, time_type>(val1, h2));
      if (i == m_steps / 2 - 1)
    ...
    // x = 0.5*( m_x0 + m_x1 + h*m_dxdt )
    m_algebra.for_each4(
        out, m_x0.m_v, m_x1.m_v, derivs[m_steps - 1].m_v,
        typename operations_type::template scale_sum3<value_type, value_type,
                                                      time_type>(val05, val05,
                                                                 val05 * h));
  }
