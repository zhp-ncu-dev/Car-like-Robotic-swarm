/*
 ...
 [begin_description]
 Impementation of the implicit Euler method. Works with ublas::vector as state
 type. [end_description]
...
class implicit_euler {
 public:
  ...
  void do_step(System system, state_type &x, time_type t, time_type dt) {
    ...
    for (size_t i = 0; i < x.size(); ++i) m_pm.m_v[i] = i;
    ...
    m_jacobi.m_v *= dt;
    m_jacobi.m_v -=
        boost::numeric::ublas::identity_matrix<value_type>(x.size());
    ...
    // ToDo: maybe we should apply only one Newton step -> linear implicit
    // one-step scheme
    while (boost::numeric::ublas::norm_2(m_b.m_v) > m_epsilon) {
      ...
      //            m_jacobi *= dt;
      //            m_jacobi -= boost::numeric::ublas::identity_matrix<
      //            value_type >( x.size() );
 ...
 private:
  ...
  bool resize_impl(const StateIn &x) {
    ...
    resized |= adjust_size_by_resizeability(
        m_x, x, typename is_resizeable<state_type>::type());
    resized |= adjust_size_by_resizeability(
        m_b, x, typename is_resizeable<deriv_type>::type());
    resized |= adjust_size_by_resizeability(
        m_jacobi, x, typename is_resizeable<matrix_type>::type());
    resized |= adjust_size_by_resizeability(
