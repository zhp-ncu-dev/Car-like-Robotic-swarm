...
class bulirsch_stoer_dense_out {
 public:
  ...
  typedef bulirsch_stoer_dense_out<State, Value, Deriv, Time, Algebra,
                                   Operations, Resizer>
  ...
  typedef std::vector<inv_time_type>
      inv_time_vector;  // should be 1/time_type for boost.units
  ...
  bulirsch_stoer_dense_out(value_type eps_abs = 1E-6, value_type eps_rel = 1E-6,
    ...
    for (unsigned short i = 0; i < m_k_max + 1; i++) {
      ...
      for (size_t k = 0; k < i; ++k) {
        ...
        m_coeff[i][k] =
            1.0 / (r * r - static_cast<value_type>(
                               1.0));  // coefficients for extrapolation
      }
      // crude estimate of optimal order

      m_current_k_opt = 4;
      /* no calculation because log10 might not exist for value_type!
      const value_type logfact( -log10( max BOOST_PREVENT_MACRO_SUBSTITUTION(
      eps_rel , static_cast< value_type >( 1.0E-12 ) ) ) * 0.6 + 0.5 );
      m_current_k_opt = max BOOST_PREVENT_MACRO_SUBSTITUTION( 1 , min
      BOOST_PREVENT_MACRO_SUBSTITUTION( static_cast<int>( m_k_max-1 ) ,
      static_cast<int>( logfact ) ));
      */
    }
    int num = 1;
  ...
  controlled_step_result try_step(System system, const StateIn &in,
                                  ...
                                  StateOut &out, DerivOut &dxdt_new,
                                  time_type &dt) {
    BOOST_USING_STD_MIN();
    BOOST_USING_STD_MAX();
    using std::pow;
    ...
    // std::cout << "t=" << t <<", dt=" << dt << ", k_opt=" << m_current_k_opt
    // << ", first: " << m_first << std::endl;

    for (size_t k = 0; k <= m_current_k_opt + 1; k++) {
      m_midpoint.set_steps(m_interval_sequence[k]);
      if (k == 0) {
        m_midpoint.do_step(sys, in, dxdt, t, out, dt, m_mp_states[k].m_v,
                           m_derivs[k]);
      } else {
        m_midpoint.do_step(sys, in, dxdt, t, m_table[k - 1].m_v, dt,
                           m_mp_states[k].m_v, m_derivs[k]);
        ...
        const value_type error =
            m_error_checker.error(m_algebra, in, dxdt, m_err.m_v, dt);
        ...
        if ((k == m_current_k_opt - 1) ||
            m_first) {  // convergence before k_opt ?
          if (error < 1.0) {
            // convergence
            reject = false;
            if ((work[k] < KFAC2 * work[k - 1]) || (m_current_k_opt <= 2)) {
              ...
              m_current_k_opt = min BOOST_PREVENT_MACRO_SUBSTITUTION(
                  ...
                  max BOOST_PREVENT_MACRO_SUBSTITUTION(
                      2, static_cast<int>(k) + 1));
              new_h = h_opt[k] * static_cast<value_type>(m_cost[k + 1]) /
        ...
        if (k == m_current_k_opt) {  // convergence at k_opt ?
          if (error < 1.0) {
            ...
            if ((work[k - 1] < KFAC2 * work[k])) {
              m_current_k_opt = max BOOST_PREVENT_MACRO_SUBSTITUTION(
                  2, static_cast<int>(m_current_k_opt) - 1);
              new_h = h_opt[m_current_k_opt];
            } else if ((work[k] < KFAC2 * work[k - 1]) &&
                       !m_last_step_rejected) {
              m_current_k_opt = min BOOST_PREVENT_MACRO_SUBSTITUTION(
                  static_cast<int>(m_k_max) - 1,
                  static_cast<int>(m_current_k_opt) + 1);
              new_h = h_opt[k] *
                      static_cast<value_type>(m_cost[m_current_k_opt]) /
                      static_cast<value_type>(m_cost[k]);
            } else
              new_h = h_opt[m_current_k_opt];
            break;
        ...
        if (k == m_current_k_opt + 1) {  // convergence at k_opt+1 ?
          if (error < 1.0) {             // convergence
            ...
            if (work[k - 2] < KFAC2 * work[k - 1])
              m_current_k_opt = max BOOST_PREVENT_MACRO_SUBSTITUTION(
                  2, static_cast<int>(m_current_k_opt) - 1);
            if ((work[k] < KFAC2 * work[m_current_k_opt]) &&
                !m_last_step_rejected)
              m_current_k_opt = min BOOST_PREVENT_MACRO_SUBSTITUTION(
                  static_cast<int>(m_k_max) - 1, static_cast<int>(k));
            new_h = h_opt[m_current_k_opt];
          } else {
            reject = true;
            new_h = h_opt[m_current_k_opt];
    ...
    if (!reject) {
      ...
      if (error >
          static_cast<value_type>(
              10))  // we are not as accurate for interpolation as for the steps
      {
        reject = true;
        new_h =
            dt * pow BOOST_PREVENT_MACRO_SUBSTITUTION(
                     error, static_cast<value_type>(-1) / (2 * m_k_final + 2));
      } else {
        t += dt;
      }
    }
  ...
  void initialize(const StateType &x0, const time_type &t0,
    ...
    m_resizer.adjust_size(
        x0,
        detail::bind(&controlled_error_bs_type::template resize_impl<StateType>,
  ...
  /*  =======================================================
   *  the actual step method that should be called from outside (maybe make
   * try_step private?)
   */
  ...
  std::pair<time_type, time_type> do_step(System system) {
    ...
    size_t count = 0;
    while (res == fail) {
      res = try_step(system, get_current_state(), get_current_deriv(), m_t,
  ...
  time_type current_time(void) const { return m_t; }

  const state_type &previous_state(void) const { return get_old_state(); }

  time_type previous_time(void) const { return m_t_last; }

  time_type current_time_step(void) const { return m_dt; }
 ...
 private:
  ...
  // polynomial extrapolation, see http://www.nr.com/webnotes/nr3web21.pdf
  {
    ...
    m_algebra.for_each3(
        table[0].m_v, table[1].m_v, table[0].m_v,
        typename operations_type::template scale_sum2<value_type, value_type>(
  ...
  time_type calc_h_opt(time_type h, value_type error, size_t k) const {
    ...
    if (error == 0.0)
      ...
      fac = STEPFAC2 /
            pow BOOST_PREVENT_MACRO_SUBSTITUTION(error / STEPFAC1, expo);
      fac = max BOOST_PREVENT_MACRO_SUBSTITUTION(
          facmin / STEPFAC4,
          min BOOST_PREVENT_MACRO_SUBSTITUTION(1.0 / facmin, fac));
    }
    return h * fac;
  }
  ...
  bool should_reject(value_type error, size_t k) const {
    if (k == m_current_k_opt - 1) {
      ...
      const value_type d =
          m_interval_sequence[m_current_k_opt + 1] / m_interval_sequence[0];
      return (error > d * d);
    } else
      return error > 1.0;
  }
  ...
  value_type prepare_dense_output(int k, const StateIn1 &x_start,
                                  ...
                                  const StateIn2 & /* x_end */,
                                  const DerivIn2 & /*dxdt_end */, time_type dt)
  /* k is the order to which the result was approximated */
  {
     ...
     * we use 2k+3 values at the interval center theta=0 to obtain the
     * interpolation coefficients the values are x(t+dt/2) and the derivatives
     * dx/dt , ... d^(2k+2) x / dt^(2k+2) at the midpoints the derivatives are
     * approximated via finite differences all values are obtained from
     * interpolation of the results from the increasing orders of the midpoint
     * calls
     */
    ...
    for (int j = 0; j <= k; j++) {
      ...
      const value_type d =
          m_interval_sequence[j] / (static_cast<value_type>(2) * dt);
      value_type f = 1.0;  // factor 1/2 here because our interpolation interval
                           // has length 2 !!!
    ...
    for (int kappa = 0; kappa <= 2 * k + 1; kappa++) {
      ...
      // divide kappa-th derivative by kappa because we need these terms for
      // dense output interpolation
      m_algebra.for_each1(m_diffs[kappa][0].m_v,
                          typename operations_type::template scale<time_type>(
                              static_cast<time_type>(d)));

      d *= dt / (2 * (kappa + 2));
    }

    // dense output coefficients a_0 is stored in m_mp_states[0], a_i for i
    // = 1...2k are stored in m_diffs[i-1][0]

    // the error is just the highest order coefficient of the interpolation
    // polynomial this is because we use only the midpoint theta=0 as support
    // for the interpolation (remember that theta = -1 .. 1)

    value_type error = 0.0;
    if (m_control_interpolation) {
      boost::numeric::odeint::copy(m_diffs[2 * k + 1][0].m_v, m_err.m_v);
      error =
          m_error_checker.error(m_algebra, x_start, dxdt_start, m_err.m_v, dt);
    }
  ...
  void calculate_finite_difference(size_t j, size_t kappa, value_type fac,
    ...
    {
      m_algebra.for_each2(
          m_diffs[0][j].m_v, m_derivs[j][m].m_v,
      ...
      m_algebra.for_each2(
          m_diffs[kappa][j_diffs].m_v, m_derivs[j][m + kappa].m_v,
          typename operations_type::template scale_sum1<value_type>(fac));
      value_type sign = -1.0;
      int c = 1;
      // computes the j-th order finite difference for the kappa-th derivative
      // of f at t+dt/2 using function evaluations stored in m_derivs
      for (int i = m + static_cast<int>(kappa) - 2;
           i >= m - static_cast<int>(kappa); i -= 2) {
        if (i >= 0) {
          m_algebra.for_each3(
              m_diffs[kappa][j_diffs].m_v, m_diffs[kappa][j_diffs].m_v,
              m_derivs[j][i].m_v,
              typename operations_type::template scale_sum2<value_type,
                                                            value_type>(
                  1.0,
                  sign * fac *
                      boost::math::binomial_coefficient<value_type>(kappa, c)));
        } else {
          m_algebra.for_each3(
  ...
  void do_interpolation(time_type t, StateOut &out) const {
    ...
    // m_k_final is the number of order-iterations done for the last step - it
    // governs the order of the interpolation polynomial
    const value_type theta =
        2 * get_unit_value((t - m_t_last) / (m_t - m_t_last)) - 1;
    // we use only values at interval center, that is theta=0, for interpolation
    // our interpolation polynomial is thus of order 2k+2, hence we have 2k+3
    // terms

    boost::numeric::odeint::copy(m_mp_states[0].m_v, out);
    // add remaining terms: x += a_1 theta + a2 theta^2 + ... + a_{2k}
    // theta^{2k}
    value_type theta_pow(theta);
    for (size_t i = 0; i <= 2 * m_k_final + 1; ++i) {
      m_algebra.for_each3(
          out, out, m_diffs[i][0].m_v,
          typename operations_type::template scale_sum2<value_type>(
  ...
  bool resize_impl(const StateIn &x) {
    ...
    resized |= adjust_size_by_resizeability(
        m_dxdt1, x, typename is_resizeable<state_type>::type());
    resized |= adjust_size_by_resizeability(
        m_dxdt2, x, typename is_resizeable<state_type>::type());
    resized |= adjust_size_by_resizeability(
        m_err, x, typename is_resizeable<state_type>::type());
    ...
    for (size_t i = 0; i < m_k_max + 1; ++i)
      resized |= adjust_size_by_resizeability(
          m_mp_states[i], x, typename is_resizeable<state_type>::type());
    for (size_t i = 0; i < m_k_max + 1; ++i)
      for (size_t j = 0; j < m_derivs[i].size(); ++j)
        resized |= adjust_size_by_resizeability(
            m_derivs[i][j], x, typename is_resizeable<deriv_type>::type());
    for (size_t i = 0; i < 2 * m_k_max + 1; ++i)
      for (size_t j = 0; j < m_diffs[i].size(); ++j)
        resized |= adjust_size_by_resizeability(
            m_diffs[i][j], x, typename is_resizeable<deriv_type>::type());
  ...
  void toggle_current_state(void) { m_current_state_x1 = !m_current_state_x1; }

  default_error_checker<value_type, algebra_type, operations_type>
      m_error_checker;
  modified_midpoint_dense_out<state_type, value_type, deriv_type, time_type,
                              algebra_type, operations_type, resizer_type>
      m_midpoint;
  ...
  // for dense output:
  state_vector_type
      m_mp_states;  // sequence of approximations of x at distance center
...
/**
 * \fn bulirsch_stoer_dense_out::bulirsch_stoer_dense_out( value_type eps_abs ,
 * value_type eps_rel , value_type factor_x , value_type factor_dxdt , bool
 * control_interpolation ) \brief Constructs the bulirsch_stoer class, including
 * initialization of the error bounds.
 *
...
/**
 * \fn bulirsch_stoer_dense_out::try_step( System system , const StateIn &in ,
 * const DerivIn &dxdt , time_type &t , StateOut &out , DerivOut &dxdt_new ,
 * time_type &dt ) \brief Tries to perform one step.
 *
...
/**
 * \fn bulirsch_stoer_dense_out::initialize( const StateType &x0 , const
 * time_type &t0 , const time_type &dt0 ) \brief Initializes the dense output
 * stepper.
 *
