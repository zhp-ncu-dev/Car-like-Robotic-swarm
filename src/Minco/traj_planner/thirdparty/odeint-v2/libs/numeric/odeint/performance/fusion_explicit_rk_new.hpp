...
class explicit_rk {
 public:
  ...
  typedef typename fusion::result_of::as_vector<typename mpl::copy<
      ...
      mpl::inserter<
          mpl::vector0<>,
          mpl::push_back<mpl::_1, array_wrapper<double, mpl::_2> > > >::type>::
      type coef_a_type;
  ...
  struct stage_vector : public stage_vector_base {
    struct do_insertion {
      ...
      void operator()(Index) const {
        // fusion::at< Index >( m_base ) = stage< double , Index::value+1 ,
        // intermediate_stage >( m_c[ Index::value ] , fusion::at< Index >( m_a )
        // );
    ...
    stage_vector(const coef_a_type &a, const coef_b_type &b,
      ...
      // fusion::at_c< 0 >( fusion::at_c< stage_count - 1 >( *this ) ) =
      // stage_count - 1 ;
  ...
  struct calculate_stage {
    ...
    calculate_stage(System &_system, state_type &_x, state_type &_x_tmp,
                    state_type *_F, const double _t, const double _dt)
        : system(_system), x(_x), x_tmp(_x_tmp), F(_F), t(_t), dt(_dt) {}

    template <typename T, size_t stage_number>
    void inline operator()(
        stage<T, stage_number, intermediate_stage> const &stage) const
    ...
    template <typename T, size_t stage_number>
    void inline operator()(
        stage<T, stage_number, last_stage> const &stage) const
    // void operator()( typename stage_fusion_wrapper< T , mpl::size_t<
    // stage_number > , last_stage >::type const &stage ) const
    {
      if (stage_number == 1)
        system(x, F[stage_number - 1], t + stage.c * dt);
      else
 ...
 public:
  ...
  template <class System>
  void inline do_step(System system, state_type &x, const double t,
                      const double dt) {
    fusion::for_each(m_stages,
                     calculate_stage<System>(system, x, m_x_tmp, m_F, t, dt));
  }
