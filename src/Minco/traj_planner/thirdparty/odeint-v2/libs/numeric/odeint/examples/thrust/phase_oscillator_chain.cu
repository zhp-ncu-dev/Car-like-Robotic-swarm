 ...
 public:
  struct sys_functor {
    ...
    __host__ __device__ void operator()(
        Tuple t)  // this functor works on tuples of values
    {
  ...
  void operator()(const state_type &x, state_type &dxdt, const value_type dt) {
    thrust::for_each(
        ...
        thrust::make_zip_iterator(thrust::make_tuple(
            x.end(), thrust::make_permutation_iterator(x.begin(), m_prev.end()),
            thrust::make_permutation_iterator(x.begin(), m_next.end()),
            m_omega.end(), dxdt.end())),
        sys_functor());
  }
...
const value_type epsilon =
    6.0 / (N * N);  // should be < 8/N^2 to see phase locking
...
int main(int arc, char *argv[]) {
  ...
  // create stepper
  runge_kutta4<state_type, value_type, state_type, value_type, thrust_algebra,
               thrust_operations>
      stepper;
