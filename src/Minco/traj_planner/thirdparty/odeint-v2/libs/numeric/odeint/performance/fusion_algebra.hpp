...
struct fusion_algebra {
  ...
  inline static void foreach (boost::array<T, dim> &x_tmp,
                              ...
                              const boost::array<T, dim> k_vector[n],
                              const double dt) {
    for (size_t i = 0; i < dim; ++i) {
      x_tmp[i] = x[i];  // + a[0]*dt*k_vector[0][i];
      for (size_t j = 0; j < n; ++j) x_tmp[i] += a[j] * dt * k_vector[j][i];
    }
  }
  ...
  inline static void foreach (boost::array<T, dim> &x_tmp,
    ...
    for (size_t i = 0; i < dim; ++i) {
      ...
      for (size_t j = 1; j < n; ++j) x_tmp[i] += a[j] * dt * k_vector[j][i];
    }
  }
};
...
/* !!!!!!!   Actually, this is factor 3 slower with intel compiler, so we don'y
 * use it !!!!! Update: It increases performance on msvc 9.0 by about 30%, so it
 * is activated for MSVC
 */
...
struct fusion_algebra<3> {
  ...
  inline static void foreach (boost::array<T, dim> &x_tmp,
    ...
    for (size_t i = 0; i < dim; ++i) {
      x_tmp[i] = x[i] + a[0] * dt * k_vector[0][i] +
                 a[1] * dt * k_vector[1][i] + a[2] * dt * k_vector[2][i];
    }
  }
};
...
struct fusion_algebra<5> {
  ...
  inline static void foreach (boost::array<T, dim> &x_tmp,
    ...
    for (size_t i = 0; i < dim; ++i) {
      x_tmp[i] = x[i] + a[0] * dt * k_vector[0][i] +
                 a[1] * dt * k_vector[1][i] + a[2] * dt * k_vector[2][i] +
                 a[3] * dt * k_vector[3][i] + a[4] * dt * k_vector[4][i];
    }
  }
};
