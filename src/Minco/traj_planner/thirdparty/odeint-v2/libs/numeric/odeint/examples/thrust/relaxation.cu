/*
 * Solves many relaxation equations dxdt = - a * x in parallel and for different
 * values of a. The relaxation equations are completely uncoupled.
 */
...
struct relaxation {
  ...
  relaxation(size_t N, const state_type &a) : m_N(N), m_a(a) {}

  void operator()(const state_type &x, state_type &dxdt, value_type t) const {
    thrust::for_each(thrust::make_zip_iterator(thrust::make_tuple(
                         x.begin(), m_a.begin(), dxdt.begin())),
                     thrust::make_zip_iterator(
                         thrust::make_tuple(x.end(), m_a.end(), dxdt.end())),
                     relaxation_functor());
  }
