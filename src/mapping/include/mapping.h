...
class MappingProcess {
 ...
 private:
  ...
  Eigen::Vector3i
      global_map_size_;  // global_map_size_ represents the number of the grids
                         // in each dircetion of the global map
  ...
  int number_of_points_;  // This parameter represents the number of points in
                          // each frame of the pointcloud
  bool use_pred_;
  bool have_odom_;
  ...
  std::vector<double> occupancy_buffer_;  // This buffer stores the states of
                                          // each grid in the "global" map
  ...
  void raycastProcess(
      ...
      const pcl::PointCloud<pcl::PointXYZ>::Ptr laserCloudTransformed);
