...
void MappingProcess::init(const ros::NodeHandle& nh) {
  ...
  buffer_size_ = global_map_size_(0) *
                 grid_size_y_multiply_z_;  // The size of the global map
  ...
  /*********************************************************************************/
  Local_Pointcloud_sub_.reset(
      new message_filters::Subscriber<sensor_msgs::PointCloud2>(
          nh_, lidar_topic_, 20));
  Odometry_sub_.reset(new message_filters::Subscriber<nav_msgs::Odometry>(
      nh_, odom_topic_, 100));
  pointcloud_odom_sync_.reset(
      new message_filters::Synchronizer<SyncPolicyPointcloud_Odom>(
          SyncPolicyPointcloud_Odom(100), *Local_Pointcloud_sub_,
          *Odometry_sub_));
  pointcloud_odom_sync_->registerCallback(
      boost::bind(&MappingProcess::OdometryAndPointcloud_cb, this, _1, _2));
  // local_occ_vis_timer_ = nh_.createTimer(ros::Duration(0.3),
  // &MappingProcess::localOccVis_cb, this);
  global_occ_vis_timer_ = nh_.createTimer(
      ros::Duration(0.3), &MappingProcess::globalOccVis_cb, this);

  MapCloud.reset(new pcl::PointCloud<pcl::PointXYZ>);

  PointCloud_Odometry_pub_ = nh_.advertise<sensor_msgs::PointCloud2>(
      "/mapping/PointCloud_Odometry", 1);
  curr_view_cloud_pub_ =
      nh_.advertise<sensor_msgs::PointCloud2>("/mapping/local_view_cloud", 1);
  global_view_cloud_pub_ =
      nh_.advertise<sensor_msgs::PointCloud2>("mapping/global_view_cloud", 1);

  std::cout << "MappingProcess Initialized!\n" << endl;
  ;
}

void MappingProcess::OdometryAndPointcloud_cb(
    const sensor_msgs::PointCloud2::ConstPtr& pcl_msg,
  ...
  Eigen::Vector3d Position_XYZ(odom_msg->pose.pose.position.x,
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr laserCloudTransformed(
  ...
  // std::cout<<"quartenion: "<<curr_q_.w()<<"    "<<curr_q_.x()<<"
  // "<<curr_q_.y()<<"    "<<curr_q_.z()<<std::endl;
  ...
  for (int i = 0; i < number_of_points_; i++) {
    ...
    LaserCloudTransformed_XYZ =
        Rotation_matrix * LaserCloudIn_XYZ + center_position_;
    laserCloudTransformed->points.emplace_back(pcl::PointXYZ(
        LaserCloudTransformed_XYZ(0), LaserCloudTransformed_XYZ(1),
        LaserCloudTransformed_XYZ(2)));
  }

  /****** This part is used to build the pointcloud of the global map, but for
   * TGK-planner, we donot need it right now********/
  ...
  /*This part is used to publish each frame of the laser pointcloud
   * transformed*/
  ...
  raycastProcess(
      center_position_,
      laserCloudTransformed);  // center_position_ is the postion of the lidar

  // cout<< "The number of the grids which haven't been detected is :" <<
  // count(occupancy_buffer_.begin(), occupancy_buffer_.end(), -1) << endl;
  // ros::Time t2 = ros::Time::now();
  // cout<<"Time cost:    "<<(t2 - t1).toSec()<<endl;
}

/*----------This function is used to display the local grid map in
 * rviz---------------*/
  ...
  for (int x = min_id(0); x < max_id(0); ++x)
    for (int y = min_id(1); y < max_id(1); ++y)
      for (int z = min_id(2); z < max_id(2); ++z) {
        if (occupancy_buffer_[x * grid_size_y_multiply_z_ +
                              y * global_map_size_(2) + z] >
            min_occupancy_log_) {
...
void MappingProcess::globalOccVis_cb(const ros::TimerEvent& e) {
  ...
  //         {
  //         //cout << "p(): " << occupancy_buffer_[x * grid_size_y_multiply_z_
  //         + y * grid_size_(2) + z] << endl;
  //             if (occupancy_buffer_[x * grid_size_y_multiply_z_ + y *
  //             global_map_size_(2) + z] > min_occupancy_log_)
  //             // if (occupancy_buffer_[x * grid_size_y_multiply_z_ + y *
  //             global_map_size_(2) + z] != -1 && occupancy_buffer_[x *
  //             grid_size_y_multiply_z_ + y * global_map_size_(2) + z] <
  //             min_occupancy_log_)
  //             // if (occupancy_buffer_[x * grid_size_y_multiply_z_ + y *
  //             global_map_size_(2) + z] == -1)
  //             {
  //                 Eigen::Vector3i idx(x,y,z);
  //                 Eigen::Vector3d pos;
  //                 indexToPos(idx, pos);
...
void MappingProcess::raycastProcess(
    const Eigen::Vector3d& t_wc,
    const pcl::PointCloud<pcl::PointXYZ>::Ptr
        laserCloudTransformed)  // t_wc is the position of the lidar
{
  if (number_of_points_ == 0) return;
  ...
  for (int i = 0; i < number_of_points_; i++) {
    ...
    if (have_received_freespaces_) {
      for (int car = 0; car < cars_num_; car++) {
        ...
        if ((Eigen::Vector2d(pt_w.head(2)) -
             Eigen::Vector2d(normalVec_and_points.col(0).tail(2)))
                    .dot(normalVec_and_points.col(0).head(2)) <= 0 &&
            (Eigen::Vector2d(pt_w.head(2)) -
             Eigen::Vector2d(normalVec_and_points.col(1).tail(2)))
                    .dot(normalVec_and_points.col(1).head(2)) <= 0 &&
            (Eigen::Vector2d(pt_w.head(2)) -
             Eigen::Vector2d(normalVec_and_points.col(2).tail(2)))
                    .dot(normalVec_and_points.col(2).head(2)) <= 0 &&
            (Eigen::Vector2d(pt_w.head(2)) -
             Eigen::Vector2d(normalVec_and_points.col(3).tail(2)))
                    .dot(normalVec_and_points.col(3).head(2)) <= 0) {
          inside_car = true;
          break;
        }
      }
    }
    if (inside_car) continue;
    ...
    if (!need_ray) continue;
    Eigen::Vector3d half = Eigen::Vector3d(0.5, 0.5, 0.5);
    Eigen::Vector3d ray_pt;
    if (!raycaster.step(ray_pt)) continue;
  ...
  while (!cache_voxel_.empty()) {
    ...
    int idx_ctns = idx(0) * grid_size_y_multiply_z_ +
                   idx(1) * global_map_size_(2) + idx(2);
    cache_voxel_.pop();

    double log_odds_update =
        cache_hit_[idx_ctns] >= cache_all_[idx_ctns] - cache_hit_[idx_ctns]
            ? prob_hit_log_
            : prob_miss_log_;
    cache_hit_[idx_ctns] = cache_all_[idx_ctns] = 0;

    if ((log_odds_update >= 0 &&
         occupancy_buffer_[idx_ctns] >= clamp_max_log_) ||
        (log_odds_update <= 0 && occupancy_buffer_[idx_ctns] <= clamp_min_log_))
      continue;

    //------------------- With the "if" below, the map in the past is also
    //stored ----------------------------//
    // if(occupancy_buffer_[idx_ctns] <= min_occupancy_log_)
    // {
    occupancy_buffer_[idx_ctns] = std::min(
        std::max(occupancy_buffer_[idx_ctns] + log_odds_update, clamp_min_log_),
        clamp_max_log_);
    ...
    if (occupancy_buffer_[idx_ctns] > min_occupancy_log_) {
      ...
      for (int z = 0; z < global_map_size_(2); z++) {
        int idx_ctns_3d =
            idx(0) * grid_size_y_multiply_z_ + idx(1) * global_map_size_(2) + z;
...
int MappingProcess::setCacheOccupancy(const Eigen::Vector3d& pos, int occ) {
  ...
  int idx_ctns =
      id(0) * grid_size_y_multiply_z_ + id(1) * global_map_size_(2) + id(2);
...
inline bool MappingProcess::isInMap(const Eigen::Vector3i& id) {
  return ((id[0] | (global_map_size_[0] - 1 - id[0]) | id[1] |
           (global_map_size_[1] - 1 - id[1]) | id[2] |
           (global_map_size_[2] - 1 - id[2])) >= 0);
};
...
inline bool MappingProcess::isInLocalMap(const Eigen::Vector3i& id) {
  ...
  max_id(1) = min(global_map_size_[1], max_id(1));
  max_id(2) = min(global_map_size_[2], max_id(2));
  return (((id[0] - min_id[0]) | (max_id[0] - id[0]) | (id[1] - min_id[1]) |
           (max_id[1] - id[1]) | (id[2] - min_id[2]) | (max_id[2] - id[2])) >=
          0);
};
...
bool MappingProcess::isInMap2d(const Eigen::Vector2i& id) {
  if (id(0) < 0 || id(0) >= global_map_size_(0) || id(1) < 0 ||
      id(1) >= global_map_size_(1)) {
    return false;
  } else
    return true;
};

void MappingProcess::posToIndex2d(const Eigen::Vector2d& pos,
                                  Eigen::Vector2i& id) {
  for (int i = 0; i < 2; i++)
    id(i) = floor((pos(i) - origin_(i)) * resolution_inv_);
}

void MappingProcess::indexToPos2d(const Eigen::Vector2i& id,
                                  Eigen::Vector2d& pos) {
  // pos = origin_;
  for (int i = 0; i < 2; i++) pos(i) = (id(i) + 0.5) * resolution_ + origin_(i);
}

int MappingProcess::getVoxelState2d(const Eigen::Vector2d& pos) {
  Eigen::Vector2i id;
  posToIndex2d(pos, id);
  if (!isInMap2d(id)) return -1;
  // todo: add local map range

  return occupancy_buffer_2d_[id(1) * global_map_size_(0) + id(0)] > 0.5 ? 1
                                                                         : 0;
}

void MappingProcess::posToIndex(const Eigen::Vector3d& pos,
                                Eigen::Vector3i& id) {
  for (int i = 0; i < 3; i++)
    id(i) = floor((pos(i) - origin_(i)) * resolution_inv_);
}

void MappingProcess::indexToPos(const Eigen::Vector3i& id,
                                Eigen::Vector3d& pos) {
  pos = origin_;
  for (int i = 0; i < 3; i++) pos(i) = (id(i) + 0.5) * resolution_ + origin_(i);
}
...
int MappingProcess::getVoxelState(const Eigen::Vector3d& pos) {
  ...
  if (!isInLocalMap(id)) return 0;

  return occupancy_buffer_[id(0) * grid_size_y_multiply_z_ +
                           id(1) * global_map_size_(2) + id(2)] >
                 min_occupancy_log_
             ? 1
             : 0;
}

void MappingProcess::setFreeSpacesForMapping(
    const std::vector<Eigen::MatrixXd>& vec_freespaces) {
